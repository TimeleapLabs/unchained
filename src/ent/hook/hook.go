...
// function as AssetPrice mutator.
type AssetPriceFunc func(context.Context, *ent.AssetPriceMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AssetPriceFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.AssetPriceMutation); ok {
		return f(ctx, mv)
	}
...
// Mutate calls f(ctx, m).
func (f SignerFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.SignerMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SignerMutation", m)
}
