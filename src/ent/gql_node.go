...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case correctnessreport.Table:
		...
		return n, nil
	case eventlog.Table:
		query := c.EventLog.Query().
			Where(eventlog.ID(id))
		query, err := query.CollectFields(ctx, eventlogImplementors...)
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
		...
		return n, nil
	case signer.Table:
		query := c.Signer.Query().
			Where(signer.ID(id))
		query, err := query.CollectFields(ctx, signerImplementors...)
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
