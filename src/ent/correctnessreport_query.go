...
func (crq *CorrectnessReportQuery) loadSigners(ctx context.Context, query *SignerQuery, nodes []*CorrectnessReport, init func(*CorrectnessReport), assign func(*CorrectnessReport, *Signer)) error {
	...
	nodeids := make(map[int]*CorrectnessReport)
	for i := range nodes {
	...
	query.withFKs = true
	query.Where(predicate.Signer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(correctnessreport.SignersColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.correctness_report_signers
		if fk == nil {
			return fmt.Errorf(`foreign-key "correctness_report_signers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "correctness_report_signers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
