...
const setAttestations = (block, signers, price) => {
  const current = attestations.get(block) || [];
  const newSigners = [];
  ...
  if (newSigners.length) {
    ...
    logger.info(`${total}x attestation at block ${block}: $${price}`);
  }
...
export const attest = async (data, signature, signers) => {
  ...
  const signed = blsAttest(data, signature, signers);
  setAttestations(data.block, signed?.signers || signers, price);
  return signed;
};
