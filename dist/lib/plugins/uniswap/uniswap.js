...
const cache = new Map();
const attestations = new Map(); // Replace 'any' with a more specific type if available
const earlyAttestations = new Map(); // Replace 'any' with a more specific type if available
let provider;
const CACHE_SIZE = 50;
...
const setAttestations = (request, signer, signature) => {
    ...
    const signersList = [
        ...new Set([
            ...stored.signers,
            ...early.signers.map((sig) => sig.signer),
        ]),
    ];
    if (!signersList.includes(signer)) {
