...
export const customFormat = () => {
    return format.combine(format.splat(), format.timestamp(), format.printf(({ timestamp, level, message, ...rest }) => {
        const formattedSpace = colorizer.colorize(`level_${level}`, " ");
        const formattedTimestamp = colorizer.colorize("timestamp", ` ${timestamp} `);
        const formattedLevel = colorizer.colorize(`level_${level}`, ` ${level
            .toUpperCase()
            .padEnd(7, "-")
            .replace(/([A-Z])-/, "$1 ")}-> `);
        const peers = sockets.size.toString().padStart(4, "·");
        const formattedPeers = colorizer.colorize(`level_${level}`, `[${peers}] `);
